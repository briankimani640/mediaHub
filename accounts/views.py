...
# offer user login activities
# decorators : when functions return other functions
...
def register_view(request):
    ...
    if request.method == 'POST': # user wants to register
        ...
        # if user has filled all the required inputs
        if form.is_valid():
...
def profile_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(request, 'account/profile.html', {'form': form})
